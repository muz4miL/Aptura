...
const ServiceCard = ({ icon, title, desc, tag }) => (
  <div className="group relative flex flex-col justify-between flex-shrink-0 w-[340px] h-[280px] p-7 rounded-2xl bg-[#0a0d14] border border-white/[0.06] hover:border-[#00f0ff]/20 overflow-hidden transition-all duration-500 cursor-default select-none">
    {/* Hover glow */}
    <div
      className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
      style={{
        background:
          "radial-gradient(ellipse at 20% 80%, rgba(0,240,255,0.07) 0%, transparent 60%)",
      }}
    />

    {/* Top line accent */}
    <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#00f0ff]/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

    <div className="relative z-10">
      {/* Tag + Icon row */}
      <div className="flex items-center justify-between mb-5">
        <div className="w-12 h-12 flex items-center justify-center rounded-xl bg-white/[0.04] border border-white/[0.06] group-hover:border-[#00f0ff]/20 group-hover:bg-[#00f0ff]/[0.04] transition-all duration-300">
          <Image
            src={icon}
            alt={title}
            width={28}
            height={28}
            className="w-7 h-7 object-contain opacity-60 group-hover:opacity-100 transition duration-300"
          />
        </div>
        <span className="text-[10px] font-mono tracking-[0.2em] uppercase text-[#00f0ff]/40 group-hover:text-[#00f0ff]/70 transition-colors border border-[#00f0ff]/10 px-2.5 py-1 rounded-full">
          {tag}
        </span>
      </div>

      {/* Title */}
      <h3 className="text-[17px] font-heading font-semibold text-white mb-2.5 group-hover:text-[#00f0ff] transition-colors duration-300">
        {title}
      </h3>

      {/* Description */}
      <p className="text-[#94a3b8]/80 text-[13px] leading-relaxed">
        {desc}
      </p>
    </div>
  </div>
...
const AllServices = () => {
  ...
  return (
    <section
      ref={ref}
      className="relative bg-[#050507] py-28 overflow-hidden"
    >
      {/* Background accents */}
      <div
      ...
      />
      <div
        className="absolute top-0 left-0 w-[400px] h-[400px] rounded-full opacity-[0.02] pointer-events-none"
        style={{
      ...
      <div className="relative z-10">
        ...
        <motion.div
          ...
          className="mb-14 px-6 max-w-6xl mx-auto"
        >
